 ...
 * erzeugt, das nur noch den, von Formatierungen gesï¿½uberten, Text des Dokuments,
 * sowie seine URL und seinen Titel enthï¿½lt.
 *
 * @see Document
...
public class DocumentFactory {
  ...
  /** Die maximale Lï¿½nge der Zusammenfassung. */
  ...
  /** Die Profiler, die die Bearbeitung durch die Prï¿½paratoren messen. */
   ...
   *
   * Autor: Christian Horn, der HTML-Dokument Erkennung.
   *
  ...
  public Document createDocument(RawDocument rawDocument, ErrorLogger errorLogger) {
    ...
    // Christian Horn, notwendige €nderung im Source-Code!
    if (rawDocument.isHTMLDoc())
    {
    		for (int i = 0; i < mPreparatorArr.length; i++) {
    			String preparatortitle=mPreparatorArr[i].getClass().getName();
    			if (preparatortitle.equals(new String("net.sf.regain.crawler.preparator.HtmlPreparator"))) {
    ...
    throws RegainException
  {
      ...
      for (int i = 0; i < auxiliaryFieldArr.length; i++) {
        ...
        if (regex.match(url)) {
          ...
          if (value != null) {
            ...
            if(!auxiliaryFieldArr[i].isStored()){            
            	store = Field.Store.NO;
            }
            if(!auxiliaryFieldArr[i].isIndexed()){
            	index = Field.Index.NO;
            } 
            if(!auxiliaryFieldArr[i].isTokenized()){
            	index = Field.Index.UN_TOKENIZED;
            }
            //paro: doc.add(new Field(fieldName, value, store, index, token));
    ...
    if (mCrawlerAccessController != null) {
      ...
      //paro: doc.add(Field.Text("groups", groupList.toString()));
      Reader reader = new StringReader(groupList.toString());
      doc.add(new Field("groups", reader));
    }

    // Add the URL of the document
    //paro: doc.add(Field.Keyword("url", url));
    doc.add(new Field("url", url, Field.Store.YES, Field.Index.UN_TOKENIZED));

    // Add the document's size
    int size = rawDocument.getLength();
    //paro: doc.add(Field.Keyword("size", Integer.toString(size)));
    ...
    //paro: doc.add(Field.Keyword("last-modified", lastModifiedAsString));
    doc.add(new Field("last-modified", lastModifiedAsString, Field.Store.YES, Field.Index.UN_TOKENIZED));
    ...
    if (additionalFieldMap != null) {
      ...
      while (iter.hasNext()) {
        ...
        //paro: doc.add(Field.Text(fieldName, fieldValue));
        Reader reader = new StringReader(fieldValue);
        doc.add(new Field(fieldName, reader));
      }
    }
    ...
    if (hasContent(cleanedContent)) {
      ...
      //paro: doc.add(Field.UnStored("content", cleanedContent));
      doc.add(new Field("content", cleanedContent, Field.Store.NO, Field.Index.TOKENIZED));
    } else {
      // We have no content! This is a substitude document
      // -> Add a "preparation-error"-field
      //paro: doc.add(Field.UnIndexed("preparation-error", "true"));
    ...
    if (hasContent(title)) {
      //paro: doc.add(Field.Text("title", title));
	  //doc.add(new Field("title", new StringReader(title)));
    ...
    if (hasContent(summary)) {
      //paro: doc.add(Field.Text("summary", summary));
      //doc.add(new Field("summary", new StringReader(summary)));
    ...
    if (hasContent(headlines)) {
      //paro: doc.add(Field.Text("headlines", headlines));
      doc.add(new Field("headlines", new StringReader(headlines)));
    }

    // Add the document's path
    if (path != null) {
      String asString = pathToString(path);
      //paro: doc.add(Field.UnIndexed("path", asString));
  ...
  /**
   * Gibt zurï¿½ck, ob der String einen Inhalt hat. Dies ist der Fall, wenn er
   ...
   * <p>
   * Wenn keine Zusammenfassung mï¿½glich ist, wird <code>null</code>
   * zurï¿½ckgegeben.
   *
   * @param content Der Inhalt fï¿½r den die Zusammenfassung erstellt werden soll.
   ...
   * <p>
   * Eine Analyse-Datei enthï¿½lt die Daten des Dokuments bei jedem
   * Zwischenschritt der Aufbereitung. Sie hilft die Qualitï¿½t der
   * Index-Erstellung zu prï¿½fen und wird in einem Unterverzeichnis im
  ...
  /**
   * Gibt alle Ressourcen frei, die von den Prï¿½paratoren genutzt wurden.
   * <p>
