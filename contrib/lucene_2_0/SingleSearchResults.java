 ...
 *
 * CVS information:
 *  $RCSfile: SingleSearchResults.java,v $
 *   $Source: /cvsroot/regain/regain/src/net/sf/regain/search/results/SingleSearchResults.java,v $
    ...
    if (queryText != null) {
      ...
      try {
        ...
        for (int i = 0; i < searchFieldArr.length; i++) {
          ...
          //paro: parser.setOperator(QueryParser.DEFAULT_OPERATOR_AND);
          parser.setDefaultOperator(QueryParser.Operator.AND);
          Query fieldQuery = parser.parse(queryText);

          // Add as OR
          //paro: query.add(fieldQuery, false, false);
      ...
      if (groupArr != null) {
        ...
        for (int i = 0; i < groupArr.length; i++) {
          // Add as OR
          //paro: groupQuery.add(new TermQuery(new Term("groups", groupArr[i])), false, false);
          groupQuery.add(new TermQuery(new Term("groups", groupArr[i])), BooleanClause.Occur.SHOULD);
        }
        
        // Create a main query that contains the group query and the search query
        // combined with AND
        BooleanQuery mainQuery = new BooleanQuery();
        //paro: mainQuery.add(query, true, false);
        mainQuery.add(query, BooleanClause.Occur.MUST);
        //paro: mainQuery.add(groupQuery, true, false);
        mainQuery.add(groupQuery, BooleanClause.Occur.MUST);
